<header x-data="{ isOpen: false }" class="fixed w-full z-50 bg-light-bg dark:bg-dark-bg dark:shadow-black shadow-md">
            ...
            <div class="flex-shrink-0 flex items-center">
                <h1 class="text-2xl font-bold text-light-primary dark:text-dark-primary">
                    <a href="{{ route('home') }}" class="flex items-center">
                        <svg class="w-8 h-8 mr-2 text-light-primary dark:text-dark-primary" fill="none"
            ...
            <nav class="hidden md:flex space-x-4 items-center sm:-my-px sm:ms-10 sm:flex">
                ...
                @auth
                    ...
                    <form action="{{ route('auth.logout') }}" method="POST">
                        ...
                        @method('POST')
                        <button type="submit"
                            class="ml-4 px-4 py-2 rounded-lg text-sm font-medium text-white bg-red-600 hover:bg-red-700 transition-colors duration-300">
                            {{ __('Déconnexion') }}
                        </button>
                    </form>
                @endauth
    ...
    <div x-show="isOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 grid grid-cols-1">
            <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('home')" :active="request()->routeIs('home')">
                {{ __('Accueil') }}
            </x-nav-link>

            @guest
                <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('client.catalogue')" :active="request()->routeIs('client.catalogue')">
                    {{ __('Catalogue') }}
                </x-nav-link>
                <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('auth.login.show')" :active="request()->routeIs('auth.login.show')">
                    {{ __('Connexion') }}
                </x-nav-link>
                <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('auth.register.show')" :active="request()->routeIs('auth.register.show')">
                    {{ __('Inscription') }}
                </x-nav-link>
            @endguest
            ...
            @auth
                @if (Auth::user()->role === 'Bibliothécaire')
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('librarian.dashboard')"
                        :active="request()->routeIs('librarian.dashboard')">
                        {{ __('Tableau de bord') }}
                    </x-nav-link>
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('librarian.emprunts.index')"
                        :active="request()->routeIs('librarian.emprunts.index')">
                        {{ __('Emprunts') }}
                    </x-nav-link>
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('librarian.books.index')"
                        :active="request()->routeIs('librarian.books.index')">
                        {{ __('Livres') }}
                    </x-nav-link>
                @endif

                @if (Auth::user()->role === 'Client')
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('client.dashboard')"
                        :active="request()->routeIs('client.dashboard')">
                        {{ __('Tableau de bord') }}
                    </x-nav-link>
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('client.catalogue')"
                        :active="request()->routeIs('client.catalogue')">
                        {{ __('Catalogue') }}
                    </x-nav-link>
                    <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('client.emprunt.show')"
                        :active="request()->routeIs('client.emprunt.show')">
                        {{ __('Mes emprunts') }}
                    </x-nav-link>
                @endif

                <x-nav-link class="block px-3 py-2 rounded-md text-base font-medium" :href="route('auth.profile.show')" :active="request()->routeIs('auth.profile.show')">
                    {{ __('Profil') }}
                </x-nav-link>
                ...
                <form action="{{ route('auth.logout') }}" method="POST">
                    ...
                    @method('POST')
                    <button type="submit"
                        class="w-full text-left px-3 py-2 rounded-md text-base font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                        {{ __('Déconnexion') }}
                    </button>
                </form>
            @endauth
